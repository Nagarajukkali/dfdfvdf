...
Feature:E2E scenario for offer-canstarblue campaign
  ...
  Scenario Outline: Validate complete data for offer-canstarblue campaign for QLD - new non moving customer
    ...
    Examples:
      ...
      | New            | ELE      | 12          | offer-canstarblue | E2E_Campaign_offer-canstarblue_QLD_new_nonmoving | QLD   | Quote Tool   | COR     | No  | No | RES          | New           |
  ...
  Scenario Outline: Validate complete data for offer-canstarblue campaign for SA - new moving
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign          | folderName                                   | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | ELE      | 6           | 15          | offer-canstarblue | E2E_Campaign_offer-canstarblue_SA_new_moving | SA    | Quote Tool   | Move Home | No  | No | RES          | New           |
  @DR21.10.2.campaign
  ...
  @DR21.11.4.campaign
  Scenario Outline: Validate complete data for offer-canstarblue campaign for NSW-Essential existing non moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
