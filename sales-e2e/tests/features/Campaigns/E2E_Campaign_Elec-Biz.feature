...
Feature:E2E scenario for elec-biz campaign
  ...
  Scenario Outline: Validate complete data for elec-biz campaign for NSW Ausgrid for existing customer
    Given user has opened the '<campaign>' link in a browser and creates 'E2E_Campaign_elec-biz_Ex_NSW_Ausgrid' to save evidences
    When user provides "2042" and clicks on show me plan link
    Then user is presented with the plans
    And user validates "ELE" discount to be "<eleDiscount>" percent
    And user validates the data on plans page for "<campaign>"
    ...
    And user provides all other details on qualifier page
      | customerType | connectionAddress                   | movingType | propertyType | solarOption |
      | BUS          | 73-75 Enmore Road, NEWTOWN NSW 2042 | Non-Moving     | Renter       | No          |
    And user provides all details on checkout details page
      | customerType | journey | firstName | lastName | businessType |
      | BUS          | BUS     | test      | test     | ABN          |
    And user selects mailing address option
