...
Feature:E2E scenario for total campaign
  ...
  Scenario Outline: Validate complete data for total campaign for QLD - new moving customer
    ...
    Examples:
      ...
      | New            | ELE      | 12          | total    | E2E_Campaign_total_QLD_new_moving | QLD   | Quote Tool   | Move Home | No  | No | RES          | New           |
  ...
  Scenario Outline: Validate complete data for total campaign for NSW-Endeavour - new non moving customer
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign | folderName                           | state | sourceSystem | journey | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | 16          | 16          | total    | E2E_Campaign_total_NSW_new_nonmoving | NSW   | Quote Tool   | COR     | No  | No | RES          | New           |
  ...
  @DR22.5.5.campaign
  Scenario Outline: Validate complete data for total campaign for VIC existing moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    When user provides "3000" for postcode and proceed to view the plans
    Then user is presented with the plans
    And user validates "ELE" discount to be "<eleDiscount>" percent
  ...
  @DR22.5.5.campaign
  Scenario Outline: Validate complete data for total campaign for NSW-Endeavour existing non moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
...
#    And user moves on to fill the qualifier
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign | folderName                                 | state | sourceSystem | journey     | AAH | DD | customerType | newOrExisting        |
      | Existing       | BOTH     | 16          | 16          | total    | E2E_Campaign_total_NSW_existing_non-moving | NSW   | Quote Tool   | Plan Switch | No  | No | RES          | Existing  non-moving |
  ...
  Scenario Outline: Validate complete data for total campaign for QLD existing non moving
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | campaign | folderName                                 | state | sourceSystem | journey     | AAH | DD | customerType | newOrExisting        |
      | Existing       | ELE      | 12          | total    | E2E_Campaign_total_QLD_existing_non-moving | QLD   | Quote Tool   | Plan Switch | No  | No | RES          | Existing  non-moving |
