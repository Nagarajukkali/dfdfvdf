...
Feature:E2E scenario for total campaign
  ...
  Scenario Outline: Validate complete data for total campaign for NSW - new moving customer
    ...
    Examples:
      ...
      |New           |BOTH    |15         |15         |total     |E2E_Campaign_total_NSW_new_moving  |NSW  |Quote Tool    |Move Home|No   |No  |RES          |New            |

  Scenario Outline: Validate complete data for total campaign for QLD - new moving customer
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      ...
      |New           |ELE     |13         |total  |E2E_Campaign_total_QLD_new_moving |QLD  |Quote Tool    |Move Home|No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for total campaign for SA - new moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign   |folderName                                 |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
      |New           |BOTH    |12         |12         |total|E2E_Campaign_total_SA_new_moving |SA   |Quote Tool    |Move Home  |No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for total campaign for ACT - new moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign   |folderName                              |state |sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
      |New           |BOTH    |12         |16         |total|E2E_Campaign_cashreward_ACT_new_moving |ACT   |Quote Tool    |Move Home  |No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for total campaign for NSW existing non moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |BOTH    |15         |15         |total |E2E_Campaign_total_NSW_existing_non-moving |NSW  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |

  Scenario Outline: Validate complete data for total campaign for SA existing non moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |BOTH    |12         |12         |total |E2E_Campaign_total_SA_existing_non-moving |SA  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |

  Scenario Outline: Validate complete data for total campaign for QLD existing non moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |ELE    |13         |total |E2E_Campaign_total_QLD_existing_non-moving |QLD  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |
  ...
  Scenario Outline: Validate complete data for total campaign for ACT for existing non moving
    ...
    And user submits the quote
      ...
      And user validates below mandatory fields
        |fuelType|quoteStatus     |customerType|offerType|planCode|NMI       |renovationsSinceDeenergisation|renovationsInProgressOrPlanned|customerWithLifeSupport|lifeSupportEquipmentType|billRouteType|customerStatus    |campaign  |
        |ELE     |VERBALLYACCEPTED|RESIDENTIAL |PS      |TOPH-EA |7001073236|                              |                              |N                      |                 |EMAIL        |<customerStatus>  |<campaign>|
      And user validates all the details for 'GAS' submitted quote
      And user validates below mandatory fields
        |fuelType|quoteStatus     |customerType|offerType|planCode|MIRN      |renovationsSinceDeenergisation|renovationsInProgressOrPlanned|customerWithLifeSupport|lifeSupportEquipmentType|billRouteType|customerStatus    |campaign  |
        |GAS     |VERBALLYACCEPTED|RESIDENTIAL |PS      |TOPH-GA |5260021701|                              |                              |N                      |                        |EMAIL        |<customerStatus>  |<campaign>|
      Examples:
        |customerStatus|fuelType|eleDiscount|gasDiscount|campaign    |folderName                                      |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
        |Existing      |BOTH    |12         |16         |total        |E2E_Campaign_total_ACT_Existing_Non-moving  |ACT  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing       |
