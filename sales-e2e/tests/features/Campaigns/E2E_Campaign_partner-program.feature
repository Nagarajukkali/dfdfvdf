...
Feature:E2E scenario for partner-program
  ...
  Scenario Outline: Validate complete data for partner-program campaign for SA for new moving customer
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign         | folderName                       | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | 15          | 15          | partner-program | E2E_Campaign_partner-program_SA | SA    | Quote Tool   | Move Home | No  | No | RES          | New           |
  ...
  Scenario Outline: Validate complete data for partner-program campaign for SA existing non moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign        |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |BOTH    |15         |15         |partner-program |E2E_Campaign_partner-program_SA_existing_non-moving |SA  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |
