...
Feature:E2E scenario for offer-canstarblue campaign
  @campaign21.2.3
  Scenario Outline: Validate complete data for offer-canstarblue campaign for NSW - new moving customer
    ...
    Examples:
      ...
      |New           |BOTH    |18         |16         |offer-canstarblue     |E2E_Campaign_offer-canstarblue_NSW_new_moving  |NSW  |Quote Tool    |Move Home|No   |No  |RES          |New            |

  @campaign21.2.3
  Scenario Outline: Validate complete data for offer-canstarblue campaign for QLD - new non moving customer
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    And user provides all other details on qualifier page
      |customerType |connectionAddress                   |movingType |propertyType |solarOption  |
      |RES          |36 Cypress Street, KULUIN QLD 4558  |Non-Moving |Renter       |No           |
    And user provides all details on checkout details page
    ...
    And user validates below mandatory fields
      |fuelType|quoteStatus     |customerType|offerType|planCode|NMI       |renovationsSinceDeenergisation|renovationsInProgressOrPlanned|customerWithLifeSupport|lifeSupportEquipmentType|billRouteType|customerStatus    |campaign  |
      |ELE     |VERBALLYACCEPTED|RESIDENTIAL |COR      |TOPH-EQ |QB08556016|                              |                              |N                      |                        |EMAIL        |<customerStatus>  |<campaign>|
    Examples:
      |customerStatus|fuelType|eleDiscount|campaign           |folderName                                       |state|sourceSystem  |journey  |AAH  |DD  |customerType |newOrExisting  |
      |New           |ELE     |16         |offer-canstarblue  |E2E_Campaign_offer-canstarblue_QLD_new_nonmoving |QLD  |Quote Tool    |COR      |No   |No  |RES          |New            |

  @campaign21.2.3
  Scenario Outline: Validate complete data for offer-canstarblue campaign for SA - new moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign   |folderName                                 |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
      |New           |BOTH    |13         |13         |offer-canstarblue|E2E_Campaign_offer-canstarblue_SA_new_moving |SA   |Quote Tool    |Move Home  |No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for offer-canstarblue campaign for ACT - new moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign   |folderName                              |state |sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
      |New           |BOTH    |12         |16         |offer-canstarblue|E2E_Campaign_offer-canstarblue_ACT_new_moving |ACT   |Quote Tool    |Move Home  |No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for offer-canstarblue campaign for VIC - new moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign   |folderName                              |state |sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
      |New           |BOTH    |10         |27         |offer-canstarblue|E2E_Campaign_offer-canstarblue_VIC_new_moving |VIC   |Quote Tool    |Move Home  |No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for offer-canstarblue campaign for VIC existing non moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |BOTH    |10         |27         |offer-canstarblue |E2E_Campaign_offer-canstarblue_VIC_existing_non-moving |VIC  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |

  @campaign21.2.3
  Scenario Outline: Validate complete data for offer-canstarblue campaign for NSW existing non moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |BOTH    |18         |16         |offer-canstarblue |E2E_Campaign_offer-canstarblue_NSW_existing_non-moving |NSW  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |
  ...
  Scenario Outline: Validate complete data for offer-canstarblue campaign for SA existing non moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |BOTH    |13         |13         |offer-canstarblue |E2E_Campaign_offer-canstarblue_SA_existing_non-moving |SA  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |
  ...
  Scenario Outline: Validate complete data for offer-canstarblue campaign for QLD existing non moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |ELE    |16         |offer-canstarblue |E2E_Campaign_offer-canstarblue_QLD_existing_non-moving |QLD  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |
