...
Feature:E2E scenario for total campaign
  ...
  Scenario Outline: Validate complete data for total campaign for NSW
    ...
    Examples:
      ...
      |New           |BOTH    |17         |17         |total     |E2E_Campaign_Total_NSW  |NSW  |Quote Tool    |Move Home|No   |No  |RES          |New            |

  Scenario Outline: Validate complete data for total campaign for VIC
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign  |folderName             |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
      |New           |BOTH    |11         |27         |total     |E2E_Campaign_Total_VIC |VIC  |Quote Tool    |Plan Switch|No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for total campaign for QLD
    ...
    Examples:
      ...
      |New           |ELE     |13         |total     |E2E_Campaign_Total_QLD |QLD  |Quote Tool    |Move Home|No   |No  |RES          |New            |
