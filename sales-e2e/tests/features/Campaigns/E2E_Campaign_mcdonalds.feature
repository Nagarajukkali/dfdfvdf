...
Feature:E2E scenario for mcdonalds
  ...
  Scenario Outline: Validate complete data for mcdonalds campaign for NSW for new moving customer
    ...
    Examples:
      ...
      | New            | BOTH     | 17          | 17          | mcdonalds | E2E_Campaign_mcdonalds_NSW | NSW   | Quote Tool   | Move Home | No  | No | RES          | New           |

  Scenario Outline: Validate complete data for mcdonalds campaign for VIC for new moving customer
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    When user provides "PPMAC01" and "3188" and clicks on show me plan link
    Then user is presented with the plans
    And user validates "ELE" discount to be "<eleDiscount>" percent
    And user validates "GAS" discount to be "<gasDiscount>" percent
    And user validates the data on plans page for "<campaign>"
  ...
  Scenario Outline: Validate complete data for mcdonalds campaign for SA for new moving customer
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign         | folderName                       | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | 14           | 14          | mcdonalds | E2E_Campaign_mcdonalds_SA | SA    | Quote Tool   | Move Home | No  | No | RES          | New           |
  ...
  Scenario Outline: Validate complete data for mcdonalds campaign for QLD for new moving customer
    ...
    Examples:
      ...
      | New            | ELE      | 15          | mcdonalds | E2E_Campaign_mcdonalds_QLD | QLD   | Quote Tool   | Move Home | No  | No | RES          | New           |
