...
Feature:E2E scenario for movehouse campaign
  ...
  Scenario Outline: Validate complete data for movehouse campaign for VIC - new moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign   |folderName                              |state |sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting  |
      |New           |BOTH    |11         |27         |movehouse|E2E_Campaign_movehouse_VIC_new_moving |VIC   |Quote Tool    |Move Home  |No   |No  |RES          |New            |
  ...
  Scenario Outline: Validate complete data for movehouse campaign for VIC existing non moving
    ...
    Examples:
      |customerStatus|fuelType|eleDiscount|gasDiscount|campaign    |folderName                                       |state|sourceSystem  |journey    |AAH  |DD  |customerType |newOrExisting        |
      |Existing      |BOTH    |11         |27         |movehouse |E2E_Campaign_movehouse_VIC_existing_non-moving |VIC  |Quote Tool    |Plan Switch|No   |No  |RES          |Existing  non-moving |
