...
Feature: This feature is to test the My account scenarios for business customers
  ...
  Scenario Outline: Verify the BSME Plan switch journey from My Account
    ...
    Examples:
      ...
      |E2E_MyAccount_Business_PS |e2e_bsme_ps@test.com |U2FsdGVkX1+ftH+UunfGSZtCIUBjus/t531HwywA6fY= |ELE     |My Account     |Plan Switch  |Yes  |Yes  |

  Scenario Outline: Verify the BSME Plan switch journey from My Account where customer contact person and ABN/ACN is missing
    Given user has opened the website link in a browser and creates '<folderName>' to save evidences
    And user navigates to my account login page
    When user logs in to my account using '<username>' and '<password>'
    And user clicks on view and change plan accordion for 'ELE'
    ...
    Examples:
      |folderName                          |username                |password                                     |fuelType|sourceSystem   |journey      |AAH  |DD   |
      |E2E_MyAccount_Business_PS_MissingID |MissingID_BSME@test.com |U2FsdGVkX19c8L2pzEDdebOhgJBsSfi7fZpLS5vRkI0= |ELE     |My Account     |Plan Switch  |No   |No   |
