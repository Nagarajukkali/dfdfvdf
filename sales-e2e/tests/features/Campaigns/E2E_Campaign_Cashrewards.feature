...
Feature:E2E scenario for cashrewards campaign
  ...
  Scenario Outline: Validate complete data for cashrewards campaign for NSW-Ausgrid - new moving customer
    ...
    Examples:
      ...
      | New            | ELE      | 18          | 16          | cashrewards | E2E_Campaign_cashrewards_NSW_new_moving | NSW   | Quote Tool   | Move Home | No  | No | RES          | New           |
  ...
  Scenario Outline: Validate complete data for cashrewards campaign for SA - new moving
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign    | folderName                             | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | ELE      | 6          | 15          | cashrewards | E2E_Campaign_cashrewards_SA_new_moving | SA    | Quote Tool   | Move Home | No  | No | RES          | New           |
  @DR21.10.2.campaign
  ...
  @DR21.11.4.campaign
  Scenario Outline: Validate complete data for cashrewards campaign for NSW-Essential existing non moving
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
