...
Feature: Sanity E2E scenarios for ST & ST+1 for 1View Project
  ...
  Scenario: QTCRM019 - Submit a complex quote with all the additional optional data
    Given user has opened the website link in a browser and creates 'E2E_CRM_QTCRM019' to save evidences
    And user has navigated to 'RES' plans page
    And user have selected fuel as "BOTH"
    When user clicks on the verify modal window on 'RES' page
    And user verifies the account through verify account journey for residential customer
      |customer_type|modal_option   |elecAccountNumber|gasAccountNumber |postcode |idType |idNumber |
      |RES          |verify account |8294221000       |6010360000       |2913     |dob    |12011974 |
    When user selects 'Basic Home'
    And user moves on to fill the qualifier
    And user provides all other details on qualifier page
      |customerType |connectionAddress                          |movingType  |propertyType   |solarOption  |
      |RES          |44 Noongale Court, NGUNNAWAL ACT 2913      |Moving      |Renter         |No           |
    And user provides all details on checkout details page
      |customerType |journey    |customerStatus  |firstName     |lastName |idType         |medicareType |
      |RES          |RES        |Existing        |QTCRMNinteen  |test     |Driver License |             |
    And user opts in for Carbon Neutral
    And user opts for AAH and DD
      |optAAHOption|aahAccessLevel|optDDOption|directDebitType|
      |Yes         |Level 1       |Yes        |CC             |
  ...
  Scenario: QTCRM020 & QTCRM025 - Submit a quote with CDE response as 'Accept' and FRMP as UNKNOWN
    ...
    And user provides all other details on qualifier page
      |customerType |connectionAddress                        |movingType  |propertyType   |solarOption  |
      |RES          |4 Cadiz Street, BRAY PARK QLD 4500       |Moving      |Owner          |No           |
    And user provides all details on checkout details page
      |customerType |journey    |customerStatus  |firstName   |lastName |idType         |medicareType |
      |RES          |RES        |New             |QTCRMTwenty |test     |Driver License |             |
    And user selects answer for property renovation question for 'QLD'
    And user selects billing preference option
      |option               |otherAddress                                 |
      |Connection Address   |                                             |
    And user clicks on 'Review your order' button and navigates to review page
