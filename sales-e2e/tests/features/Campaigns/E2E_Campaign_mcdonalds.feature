...
Feature:E2E scenario for mcdonalds
  ...
  Scenario Outline: Validate complete data for mcdonalds campaign for VIC for new moving customer
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign  | folderName                 | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | 13          | 28          | mcdonalds | E2E_Campaign_mcdonalds_VIC | VIC   | Quote Tool   | Move Home | No  | No | RES          | New           |
...
@DR21.7.5.campaign
  ...
  Scenario Outline: Validate complete data for mcdonalds campaign for VIC existing non moving
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign  | folderName                                     | state | sourceSystem | journey     | AAH | DD | customerType | newOrExisting        |
      | Existing       | BOTH     | 13          | 28          | mcdonalds | E2E_Campaign_mcdonalds_VIC_existing_non-moving | VIC   | Quote Tool   | Plan Switch | No  | No | RES          | Existing  non-moving |
