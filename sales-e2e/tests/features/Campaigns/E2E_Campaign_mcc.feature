...
Feature:E2E scenario for mcc
  ...
  Scenario Outline: Validate complete data for mcc campaign for NSW for new moving customer
    ...
    Examples:
      ...
      | New            | BOTH     | 17          | 17          | mcc | E2E_Campaign_EACoporateOffer_NSW | NSW   | Quote Tool   | Move Home | No  | No | RES          | New           |

  Scenario Outline: Validate complete data for mcc campaign for VIC for new moving customer
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    When user provides "PPMCC01" and "3188" and clicks on show me plan link
    Then user is presented with the plans
    And user validates "ELE" discount to be "<eleDiscount>" percent
    And user validates "GAS" discount to be "<gasDiscount>" percent
    And user validates the data on plans page for "<campaign>"
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign         | folderName                        | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | 12          | 29          | mcc | E2E_Campaign_mcc_VIC | VIC   | Quote Tool   | Move Home | No  | No | RES          | New           |

  Scenario Outline: Validate complete data for mcc campaign for SA for new moving customer
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign         | folderName                       | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | 14           | 14          | mcc | E2E_Campaign_mcc_SA | SA    | Quote Tool   | Move Home | No  | No | RES          | New           |
  ...
  Scenario Outline: Validate complete data for mcc campaign for QLD for new moving customer
    ...
    Examples:
      ...
      | New            | ELE      | 15          | mcc | E2E_Campaign_mcc_QLD | QLD   | Quote Tool   | Move Home | No  | No | RES          | New           |
