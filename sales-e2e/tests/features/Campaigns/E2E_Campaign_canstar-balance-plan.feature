...
Feature:E2E scenario for canstar-balance-plan

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for NSW-Ausgrid for new moving customer
...
#    And user validates "GAS" discount to be "<gasDiscount>" percent for "<planName>" plan
    ...
    Examples:
      ...
      | New            | BOTH     | Balance Plan | 0           | 0           | balance  | NSW   | E2E_can-bal-plan_NSW_new_moving | Quote Tool   | Move Home | NSW   | RES          | New           | No  | No |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for QLD - new moving customer
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
    ...
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | NMI        | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   | state   |
      | ELE      | VERBALLYACCEPTED | RESIDENTIAL  | ENE       | TAPR1-EQ | QB08556016 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> | <state> |
    Examples:
      ...
      | New            | ELE      | Balance Plan | 0           | balance  | E2E_can-balance-plan_QLD_new_moving | QLD   | Quote Tool   | Move Home | No  | No | RES          | New           |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for SA - new non moving
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      ...
      | New            | BOTH     | Balance Plan | 0           | 0           | balance  | E2E_can-balance-plan_SA_new_nonmoving | SA    | Quote Tool   | COR     | No  | No | RES          | New           |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for ACT - new moving
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
    ...
    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | gasDiscount | campaign | folderName                          | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | ELE      | Balance Plan | 0           | 0           | balance  | E2E_can-balance-plan_ACT_new_moving | ACT   | Quote Tool   | Move Home | No  | No | RES          | New           |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for ACT - new non-moving
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
    When user provides "2600" and clicks on show me plan link
    And user validates the data on 'Canstar' plans page for "<campaign>" plan
      | fuelType | Feature_50Credit | Feature_carbonNeutral | Feature_peaceOfMind | Feature_variableRates | Feature_XX_discountOffTotalEnergyBill | Feature_noStandardConnectionFee | state   | Feature_moveHomeCredit | planName   |
...
#    And user selects answer for property renovation question for '<state>'
    ...
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | NMI | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | ELE      | VERBALLYACCEPTED | RESIDENTIAL  | COR       | TAPR1-EA |     |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    And user validates all the details for 'GAS' submitted quote
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | MIRN       | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | GAS      | VERBALLYACCEPTED | RESIDENTIAL  | COR       | TAPR1-GA | 5260073228 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | gasDiscount | campaign | folderName                           | state | sourceSystem | journey | AAH | DD | customerType | newOrExisting |
      | New            | ELE      | Balance Plan | 0           | 0           | balance  | E2E_can-balance-plan_ACT_new_nonmove | ACT   | Quote Tool   | COR     | No  | No | RES          | New           |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for ACT - existing non-moving
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
    When user provides "2600" and clicks on show me plan link
    And user validates the data on 'Canstar' plans page for "<campaign>" plan
      | fuelType | Feature_50Credit | Feature_carbonNeutral | Feature_peaceOfMind | Feature_variableRates | Feature_XX_discountOffTotalEnergyBill | Feature_noStandardConnectionFee | state   | Feature_moveHomeCredit | planName   |
    ...
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | NMI        | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | ELE      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-EA | 7001111161 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    And user validates all the details for 'GAS' submitted quote
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | MIRN       | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | GAS      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-GA | 5260060181 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | gasDiscount | campaign | folderName                           | state | sourceSystem | journey     | AAH | DD | customerType | newOrExisting        |
  ...
  Scenario Outline: Validate complete data for canstar-balance-plan for VIC - new non-moving
    ...
    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | gasDiscount | campaign | folderName                              | state | sourceSystem | journey | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | Balance Plan | 0           | 0           | balance  | E2E_can-balance-plan_VIC_new_non_moving | VIC   | Quote Tool   | COR     | No  | No | RES          | New           |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for NSW-Endeavour - new non moving customer
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
  ...
  Scenario Outline: Validate complete data for canstar-balance-plan for VIC existing non moving
    ...
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | NMI        | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | ELE      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-EV | 6305182017 | N                              | N                              | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    And user validates all the details for 'GAS' submitted quote
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | MIRN       | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
  ...
  Scenario Outline: Validate complete data for canstar-balance-plan for VIC existing moving
    ...
    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | gasDiscount | campaign | folderName                               | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting        |
      | Existing       | BOTH     | Balance Plan | 0           | 0           | balance  | E2E_can-balance-plan_VIC_existing_moving | VIC   | Quote Tool   | Move Home | No  | No | RES          | Existing  non-moving |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for NSW-Endeavour existing non moving
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
...
#      | <customerStatus> | 6942134043    | Postcode            | 2529             | dob    | 01011980 |
    ...
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | NMI        | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | ELE      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-EN | 4310205783 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    And user validates all the details for 'GAS' submitted quote
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | MIRN       | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | GAS      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-GN | 5240151444 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |

    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | gasDiscount | campaign | folderName                                   | state | sourceSystem | journey     | AAH | DD | customerType | newOrExisting        |
      | Existing       | BOTH     | Balance Plan | 0           | 0           | balance  | E2E_can-balance-plan_NSW_existing_non-moving | NSW   | Quote Tool   | Plan Switch | No  | No | RES          | Existing  non-moving |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for SA existing non moving
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
    ...
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | NMI        | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | ELE      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-ES | 2001219800 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    And user validates all the details for 'GAS' submitted quote
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | MIRN       | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | GAS      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-GS | 5510177350 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | gasDiscount | campaign | folderName                                  | state | sourceSystem | journey     | AAH | DD | customerType | newOrExisting        |
      | Existing       | BOTH     | Balance Plan | 0           | 0           | balance  | E2E_can-balance-plan_SA_existing_non-moving | SA    | Quote Tool   | Plan Switch | No  | No | RES          | Existing  non-moving |

  @DR23.3.3.campaign
  Scenario Outline: Validate complete data for canstar-balance-plan for QLD existing non moving
    Given user has opened the 'canstar' link in a browser and creates '<folderName>' to save evidences
    When user provides "4053" and clicks on show me plan link
    And user validates the data on 'Canstar' plans page for "<campaign>" plan
      | fuelType | Feature_50Credit | Feature_carbonNeutral | Feature_peaceOfMind | Feature_variableRates | Feature_XX_discountOffTotalEnergyBill | Feature_noStandardConnectionFee | state   | Feature_moveHomeCredit | planName   |
      | ELE      | N                | Y                     | N                   | Y                     | N                                     | N                               | <state> | N                      | <planName> |
    And user validates disclaimer on plans page for "<campaign>" plan
      | referencePriceComparison | goNeutral | solarBuyBack | planName   | state   | signUpCredit | moveHomeCredit |
      | Y                        | Y         | Y            | <planName> | <state> | N            | N              |
    When user selects '<planName>'
    ...
    And user validates below mandatory fields
      | fuelType | quoteStatus      | customerType | offerType | planCode | NMI        | renovationsSinceDeenergisation | renovationsInProgressOrPlanned | customerWithLifeSupport | lifeSupportEquipmentType | billRouteType | customerStatus   | campaign   |
      | ELE      | VERBALLYACCEPTED | RESIDENTIAL  | PS        | TAPR1-EQ | QB06811396 |                                |                                | N                       |                          | EMAIL         | <customerStatus> | <campaign> |
    Examples:
      | customerStatus | fuelType | planName     | eleDiscount | campaign | folderName                                   | state | sourceSystem | journey     | AAH | DD | customerType | newOrExisting        |
      | Existing       | ELE      | Balance Plan | 0           | balance  | E2E_can-balance-plan_QLD_existing_non-moving | QLD   | Quote Tool   | Plan Switch | No  | No | RES          | Existing  non-moving |
