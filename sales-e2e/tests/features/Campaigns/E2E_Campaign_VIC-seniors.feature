...
Feature:E2E scenario for vic-seniors campaign
  ...
  Scenario Outline: Validate complete data for vic-seniors campaign for VIC - new moving
    ...
    Examples:
      ...
      | New            | BOTH     | 14          | 35          | vic-seniors | E2E_Campaign_vicseniors_VIC_new_moving | VIC   | Quote Tool   | Move Home | No  | No | RES          | New           |

@DR22.5.4.1.campaign
Scenario Outline: Validate complete data for vic-seniors campaign for VIC - COR
    Given user has opened the '<campaign>' link in a browser and creates '<folderName>' to save evidences
    When user provides "1234" and "3000" and clicks on show me plan link
    Then user is presented with the plans
    And user validates "ELE" discount to be "<eleDiscount>" percent
    ...
    Examples:
      | customerStatus | fuelType | eleDiscount | gasDiscount | campaign    | folderName                                 | state | sourceSystem | journey   | AAH | DD | customerType | newOrExisting |
      | New            | BOTH     | 14          | 35          | vic-seniors | E2E_Campaign_vicseniors_VIC_new_non-moving | VIC   | Quote Tool   | COR       | No  | No | RES          | New           |