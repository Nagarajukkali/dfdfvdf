...
export class checkoutReviewMethod {
  ...
  public static async validateFeatures(t: any, dataTable, json: any, numOfExpectedFeatures) {
        ...
        switch (dataTable[0].state) {
          ...
          case AustralianState.NSW:
            if (t.testRun.test.name.includes('Essential')) {
              await testFunction.assertText(t, eaCheckoutReviewPage.elements.eleFeatureDiscountOffTotal, json.electricity.feature.postSelect.discountOffTotalEnergyBill.NSW.Essential);
            } else if (t.testRun.test.name.includes('Endeavour')) {
              await testFunction.assertText(t, eaCheckoutReviewPage.elements.eleFeatureDiscountOffTotal, json.electricity.feature.postSelect.discountOffTotalEnergyBill.NSW.Endeavour);
            } else if (t.testRun.test.name.includes('Ausgrid')) {
              await testFunction.assertText(t, eaCheckoutReviewPage.elements.eleFeatureDiscountOffTotal, json.electricity.feature.postSelect.discountOffTotalEnergyBill.NSW.Ausgrid);
            } else {
    ...
    if (await testFunction.isGas(dataTable[0].fuelType)) {
      ...
      if (dataTable[0].Feature_peaceOfMind === "Y") {
        if (t.testRun.test.name.includes('employee')){
        await testFunction.assertText(t, eaCheckoutReviewPage.elements.gasFeaturePeaceOfMindEE, json.gas.feature.postSelect.peaceOfMind);
        }else{
  ...
  public static async validateSolarComponent(t, solarSetUp) {
    ...
    if (solarSetUp.toLowerCase() === 'yes') {
      ...
      if (connectionAddress.includes("VIC") && pageUrl.includes("customerType=RES")) {
        ...
        await testFunction.assertText(t, eaCheckoutReviewPage.elements.solarPower.singleRateTariff, "7.1c/kWh");
